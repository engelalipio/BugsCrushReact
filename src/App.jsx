...
// ── Audio helpers ────────────────────────────────────────────────
// Assets in public/sounds/ (from original BugCrush: Bugs, hit, Ka-Ching, Chomp, Error, Ukulele)
const SOUND_FILES = {
...
function useAudio() {
  ...
  const makeAudio = (src) => {
    ...
    a.preload = "auto";
    // Required for reliable playback in iOS WKWebView (Capacitor)
    a.setAttribute("playsinline", "");
    a.setAttribute("webkit-playsinline", "");
    return a;
  };

  // iOS blocks audio until a user gesture; briefly play/mute/pause to unlock.
  const unlock = useCallback(async () => {
    if (unlockedRef.current) return;
    const all = [
      ...Object.values(sfxCache.current),
      musicRef.current,
    ].filter(Boolean);
    await Promise.all(
      all.map(async (a) => {
  ...
  useEffect(() => {
    Object.entries(SOUND_FILES).forEach(([key, src]) => {
      if (key === "music") return;
      sfxCache.current[key] = makeAudio(src);
    });
    const music = makeAudio(SOUND_FILES.music);
    music.loop = true;
    music.volume = 0.35;
  ...
  const play = useCallback(async (key) => {
    ...
    await unlock();
    const original = sfxCache.current[key];
    if (!original) return;
    const clone = makeAudio(original.src);
    clone.volume = 0.7;
    try {
      await clone.play();
    } catch {
  ...
  const setMuted = useCallback((val) => {
    ...
    if (val) { musicRef.current?.pause(); }
    else { musicRef.current?.play().catch(() => {}); }
  }, []);

  return { play, startMusic, stopMusic, setMuted, mutedRef };
}
// ────────────────────────────────────────────────────────────────
...
export default function BugBlast() {
  ...
  const { play, startMusic, stopMusic, setMuted, mutedRef } = useAudio();
  ...
  const processMatches = useCallback((b, comboN = 0) => {
    ...
    if (!any) {
      ...
      if (comboN > 1) {
        setComboShow(comboN);
        play("combo");
        setTimeout(()=>setComboShow(null),1400);
      }
      setCombo(0);
      return;
    }
  ...
  }, [level, spawnParticles, play]);

  const handleClick = useCallback((r, c) => {
    if (animating || gameOver || victory) return;
    if (board[r][c] === -2) return;

    // Start music on very first interaction (browser autoplay policy)
    if (!gameStarted.current) { gameStarted.current = true; startMusic(); }
    ...
    setMoves(mv => {
      const nv = mv - 1;
      if (nv <= 0 && score < LEVELS[level].target) {
        setTimeout(() => { setGameOver(true); play("gameover"); }, 900);
      }
      return nv;
    });
    setBoard(swapped);
    setTimeout(() => processMatches(swapped), 80);
  }, [animating, gameOver, victory, selected, board, score, level, processMatches, resetIdle, play, startMusic]);
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @keyframes modal-pop { 0%{transform:scale(0.5);opacity:0} 60%{transform:scale(1.1);opacity:1} 100%{transform:scale(1);opacity:1} }
        .modal-pop { animation: modal-pop 0.5s ease forwards; }
      ...
      <button
        ...
        style={{
          ...
          border: "2px solid rgba(180,255,120,0.4)",
